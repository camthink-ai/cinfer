...
def get_db_service(request: Request) -> DatabaseService:
    if not hasattr(request.app.state, 'db'): 
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="Database service not available.")
    return request.app.state.db

def get_config(request: Request) -> ConfigManager:
    if not hasattr(request.app.state, 'config'):
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="Configuration service not available.")
    return request.app.state.config

def get_engine_svc(request: Request) -> EngineService:
    if not hasattr(request.app.state, 'engine_service'):
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="Engine service not available.")
...
def get_model_mgr(request: Request) -> ModelManager:
    if not hasattr(request.app.state, 'model_manager'):
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="Model manager not available.")
    return request.app.state.model_manager

def get_auth_svc_dependency(request: Request) -> AuthService:
    if not hasattr(request.app.state, 'auth_service'):
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="Auth service not available.")
    return request.app.state.auth_service

def get_token_svc_dependency(request: Request) -> TokenService:
    if not hasattr(request.app.state, 'token_service'):
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="Token service not available.")
    return request.app.state.token_service

def get_request_proc(request: Request) -> RequestProcessor:
    if not hasattr(request.app.state, 'request_processor'):
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="Request processor not available.")
...
def get_queue_mgr(request: Request) -> QueueManager:
    if not hasattr(request.app.state, 'queue_manager'):
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="Queue manager not available.")
...
def get_system_monitor(request: Request) -> SystemMonitor:
    if not hasattr(request.app.state, 'system_monitor'):
        raise APIError(error=ErrorCode.COMMON_SERVICE_UNAVAILABLE, override_message="System monitor not available.")
...
async def get_current_admin_user_id(auth_result: AuthResult = Depends(get_internal_auth_result)) -> str:
    ...
    if not auth_result.user_id: 
        raise APIError(error=ErrorCode.COMMON_INTERNAL_ERROR, override_message="Admin User ID not found in token.")
